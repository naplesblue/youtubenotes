...
1. **Phase 1: Download (下载期)**
   ...
   - **RSS 回退策略**: YouTube RSS 提要偶发返回 404 或空条目。当 `feedparser` 返回空结果时，会可选地切换至 **YouTube Data API v3**（`channels.list` + `playlistItems.list`）查询频道最新视频。引入该回退的动机是 YouTube RSS 对闯发不稳定（出现 404 等异常）这一个已观测到的实际问题，需在 `.env` 中配置 `YOUTUBE_DATA_API_KEY`（可选但强烈推荐）。
...
2. **Phase 2: Analyze (分析提取期)**
   ...
   - **执行过程**:
     - 检索出未被分析过的音频并提取视频元数据。
     - **转录引擎路由 (Subtitle / Transcriber)**: 强优先尝试从 YouTube 下载带时间轴的人工字幕：
       - **汉英双语职作**: 按 `SUBTITLE_PREFERRED_LANGS`（默认：`en, en-US, zh-Hans, zh, zh-TW`）依次匹配，先英文人工字幕，再中文人工字幕。
       - **语言概率质量门**: 英文字幕检测 `english_ratio`，中文字幕检测 `zh_ratio`，两者独立、互不干扰。
       - 字幕不可用或任意质量门不达标时，展开：将原始音频经 `ffmpeg` 到 `data/audio/`，唤起基于 `Fun-ASR-Nano` 的本地进程完成全长语音带时间戳的 STT 转录。
...
- `.downloader.`
  - `downloader.py`: `feedparser` 与 `yt-dlp` 的直接交互方；内嵌 YouTube Data API v3 回退逻辑，当 RSS 喘源可用性阉断时自动切换。
- `.transcribe.`
  - `funasr.py`: 处理极长视频时使用的切片重组机制工具类（避免内存溢出），支持 Worker 持驻留机制和按需冷启。
- `.analyzer.`
  - `config.py` & `metadata.py`: 管理和推算上下文的统一常量源。
  - `subtitle.py`: 按语言优先级链抩取人工字幕（英文/中文）并解析 VTT，执行语言族质量过滤门限判定（英文检查 `english_ratio`，中文检查 `zh_ratio`）。
