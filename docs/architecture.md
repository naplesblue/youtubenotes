...
| `build_dashboard_data.py` | **Web 仪表盘数据聚合**：读取 opinions、blogger_profiles、ticker_consensus、market_cache、video results，输出单个 `web/public/data/dashboard.json` 供 Astro 前端消费。每次 pipeline/tracker 跑完后执行。 |

## 五、Web 静态仪表盘 (web/)

独立于 Obsidian 的全景数据仪表盘，纯静态构建，可部署 GitHub Pages 或任意静态服务器。

**技术栈**: Astro + React islands + Tailwind CSS v4 + Recharts + TanStack Table v8

**数据流**: `tools/build_dashboard_data.py` 聚合所有 JSON 数据 → `web/public/data/dashboard.json` → Astro 构建时读取 → 生成 43 个静态 HTML 页面到 `web/dist/`

**页面结构**:

| 页面 | 路由 | 内容 |
|------|------|------|
| 概览 | `/` | 6 个统计卡片 + 最近 30 条活动时间线 + Top 10 Ticker 柱状图 |
| 博主排行 | `/bloggers/` | TanStack Table 可排序表格（观点数/胜率/信誉分） |
| 博主详情 | `/bloggers/[analyst]/` | 情绪饼图 + 热门 Ticker + 全部观点表 |
| Ticker 网格 | `/tickers/` | 卡片网格（搜索/排序 + 情绪条 + 目标均价） |
| 个股详情 | `/tickers/[ticker]/` | Recharts 价格折线 + 观点散点叠加 + 分析师共识 |
| 数据质量 | `/quality/` | 自动检测缺失字段、别名冲突、非标情绪等 |

**刷新命令**:
```bash
python tools/build_dashboard_data.py   # 聚合最新数据
cd web && npm run build                 # 静态构建 → dist/
```
